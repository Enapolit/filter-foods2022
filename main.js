...
const actives = document.querySelectorAll(".item2.active");
actives.forEach(function (item2) {
    item.classList.remove("active");
});

const make_active = document.querySelectorAll(`.item2.${filter}`);

make_active.forEach(function (item2) {
    item.classList.add("active");
});
});
